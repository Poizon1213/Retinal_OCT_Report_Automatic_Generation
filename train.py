...
# from DenseMultiFusionV2_V3 import *
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    # Batches
    for i, (imgs, _, caps, caplens) in enumerate(train_loader):
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    labels = np.zeros((3898, 11))
    outputs = np.zeros((3898, 11))

    # Batches
    for i, (imgs, _, caps, caplens, ) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            # loss = word_loss + alpha_c * ((1. - alphas.sum(dim=1)) ** 2).mean() + label_loss
        # Keep track of metrics
        losses.update(loss.item(), sum(decode_lengths))
        top5 = accuracy(scores.data, targets.data, 5)
        top5accs.update(top5, sum(decode_lengths))
