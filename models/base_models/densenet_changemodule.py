...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(6, 12, 24, 16), num_init_features=64,
        ...
        # self.my_features1 = nn.Sequential(OrderedDict([
        #     ('conv0-0', nn.Conv2d(128, 128, 7, 2, 3, bias=False, groups=128)),
        ...
        #         nn.Sigmoid()
        # )
        # self.my_features2 = nn.Sequential(OrderedDict([
        #     ('conv0', nn.Conv2d(128, num_init_features, 7, 2, 3, bias=False)),
        #     ('norm0', norm_layer(num_init_features)),
        #     ('relu0', nn.ReLU(True)),
        #     ('pool0', nn.MaxPool2d(3, 2, 1)),
        # ]))
        ...
        self.my_features1 = nn.Sequential(OrderedDict([
            ('conv_group', nn.Conv2d(128, num_init_features, 7, 2, 3, bias=False, groups=64)),
            ('norm0', norm_layer(num_init_features)),
            ('relu0', nn.ReLU(True)),
            ('pool0', nn.MaxPool2d(3, 2, 1)),
        ]))
        ...
        self.my_features2 = nn.Sequential(OrderedDict([
            ('conv0-0', nn.Conv2d(128, num_init_features, 7, 2, 3, bias=False)),
            ('norm0', norm_layer(num_init_features)),
            ('relu0', nn.ReLU(True)),
            ('pool0', nn.MaxPool2d(3, 2, 1)),
        ]))
        self.features = nn.Sequential(OrderedDict([
            # ('conv0-1', nn.Conv2d(128, num_init_features, 7, 2, 3, bias=False)),
            # ('norm0', norm_layer(num_init_features)),
            # ('relu0', nn.ReLU(True)),
            # ('pool0', nn.MaxPool2d(3, 2, 1)),
        ]))
        ...
        # self.features_matrix = [nn.Sequential()] * len(block_config)
        # self.group_features_metrix = [nn.Sequential()] * len(block_config)
    ...
    def extract_features(self, x):
        # x = self.features(x)
        # return x
        # x_1 = self.my_features1(x)
        # b, c, _, _ = x_1.size()
        # x_1 = self.avg_pool(x_1).view(b, c)
        # x_1 = self.my_fc(x_1).view(b, c, 1, 1)
        # x = self.features(x * x_1 + x)
        x_add1 = self.my_features1(x)
        x_add2 = self.my_features2(x)
        x = self.features(x_add1 + x_add2)
        return x
...
def get_densenet(num_layers, pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load('model_densenet121_param.pth')
